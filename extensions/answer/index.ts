...
const MINI_MODEL_ID = "gpt-5.4-mini";

/**
 * Prefer Mini for extraction (fast, cheap), otherwise fallback to the current model.
 */
async function selectExtractionModel(
  ...
  const miniModel = modelRegistry.find("openai-codex", MINI_MODEL_ID);
  if (miniModel) {
    const auth = await modelRegistry.getApiKeyAndHeaders(miniModel);
    if (auth.ok) {
      return miniModel;
    }
  }
...
class QnAComponent implements Component {
  ...
  private formatContextForDisplay(context: string, maxWidth: number): string[] {
    ...
    if (matches.length >= 2) {
      ...
      for (let i = 0; i < matches.length; i++) {
        ...
        const endIndex =
          i < matches.length - 1 ? matches[i + 1].index : context.length;
        const choiceText = context
  ...
  render(width: number): string[] {
    ...
    if (q.context) {
      lines.push(padToWidth(emptyBoxLine()));
      const contextLines = this.formatContextForDisplay(
        q.context,
        contentWidth - 4,
      );
